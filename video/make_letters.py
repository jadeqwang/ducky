...
The face, approximating the Pixar wordmark:
  ...
  - matte black, matching the classic wordmark rather than the duck yellow
...
HEADER = '''<mujoco model="microduck_props_micro">
    <!-- Props for the "MICRO" title shot.  GENERATED by video/make_letters.py —
         ...
         Sizing (duck stands 0.2511 m at the STAND keyframe):
           ...
           rubber duck   0.229 m tall = ~91% of Microduck height
         ...
         Only the I is a free body; see LETTERS in make_letters.py. The rubber
         duck is also fixed: its rounded bath-toy body otherwise rolls onto its
         side before the first shot, and nothing in the scene needs to move it.
    ...
    <default>
        <default class="letter">
            <geom type="box" rgba="0.015 0.015 0.012 1" density="180"
...
# The rubber duck stands just after the O, facing back toward the word.  Its
# long axis is therefore visible to the front cameras: the audience gets the
# unmistakable profile of a toy duck instead of looking straight into its
# beak.  Primitive overlaps are deliberate and give it the softly moulded,
# one-piece silhouette of a bath toy.
DUCK = f'''
        <!-- ============ rubber duck ============
             In line with the word, one letter-pitch after the O.  The bill
             points toward +Y (back toward the letters), presenting a clean
             profile to cameras on -X.  About 0.23 m tall: visibly larger than
             the old prop, but still shorter than Microduck. -->
        <body name="rubber_duck" pos="0 {-3.15 * PITCH:.3f} 0.0" euler="0 0 1.5708">
            <geom type="ellipsoid" name="rd_body" pos="-0.010 0 0.071"
                  size="0.082 0.058 0.066" rgba="1.0 0.78 0.035 1" density="115"
                  friction="0.9 0.02 0.002"/>
            <geom type="ellipsoid" name="rd_chest" pos="0.045 0 0.105"
                  size="0.050 0.052 0.065" rgba="1.0 0.80 0.035 1" density="115"/>
            <geom type="ellipsoid" name="rd_head" pos="0.048 0 0.179"
                  size="0.052 0.050 0.050" rgba="1.0 0.82 0.045 1" density="115"/>
            <geom type="ellipsoid" name="rd_beak" pos="0.105 0 0.171"
                  size="0.038 0.029 0.015" rgba="1.0 0.34 0.025 1" density="115"/>
            <geom type="ellipsoid" name="rd_beak_tip" pos="0.131 0 0.169"
                  size="0.018 0.025 0.010" rgba="1.0 0.27 0.018 1" density="115"/>
            <geom type="ellipsoid" name="rd_tail" pos="-0.087 0 0.105"
                  size="0.041 0.036 0.026" rgba="1.0 0.78 0.035 1" density="115"
                  euler="0 -0.48 0"/>
            <geom type="ellipsoid" name="rd_wing_l" pos="-0.010 0.053 0.091"
                  size="0.052 0.010 0.035" rgba="0.94 0.67 0.020 1"
                  contype="0" conaffinity="0"/>
            <geom type="ellipsoid" name="rd_wing_r" pos="-0.010 -0.053 0.091"
                  size="0.052 0.010 0.035" rgba="0.94 0.67 0.020 1"
                  contype="0" conaffinity="0"/>
            <geom type="sphere" name="rd_eye_l" pos="0.075 0.043 0.195"
                  size="0.0085" rgba="0.012 0.010 0.008 1" contype="0" conaffinity="0"/>
            <geom type="sphere" name="rd_eye_r" pos="0.075 -0.043 0.195"
                  size="0.0085" rgba="0.012 0.010 0.008 1" contype="0" conaffinity="0"/>
        </body>
    </worldbody>
</mujoco>
